...
export function HowItWorks() {
  return <section id="how-it-works" className="py-24 px-6 border-[1px] shadow-[0px_4px_16px_0px_#FFFFFFFF]">
      <div className="max-w-5xl mx-auto 0, 0, 1)]">
        <motion.div initial={{
        opacity: 0,
        y: 20
      }} whileInView={{
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
          ...
          duration: 0.5,
          delay: index * 0.15
        }} className="relative flex flex-col items-center text-center 0, 0, 1)]">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-6 border border-[var(--card-border)] relative z-10" style={{
            background: `color-mix(in srgb, ${step.color} 10%, var(--background))`,
            boxShadow: `0 0 30px color-mix(in srgb, ${step.color} 20%, transparent)`
          }}>
                <step.icon size={26} style={{
              color: step.color
            }} />
              </div>

              <span className="text-xs font-bold tracking-widest mb-3 uppercase" style={{
            color: step.color
          }}>
                Step {step.step}
              </span>
